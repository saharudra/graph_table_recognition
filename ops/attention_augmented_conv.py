...
class AugmentedConv(nn.Module):
    ...
    def forward(self, x):
        ...
        print("Shape of conv_out: {}".format(conv_out.shape))
        flat_q, flat_k, flat_v, q, k, v = self.compute_flat_qkv(x, self.dk, self.dv, self.Nh)
        print("Printing shape post compute_flat_qkv")
        print(flat_q.shape, flat_k.shape, flat_v.shape, q.shape, k.shape, v.shape)
    ...
    def compute_flat_qkv(self, x, dk, dv, Nh):
        qkv = self.qkv_conv(x)
        print("Shape of qkv: {}".format(qkv.shape))
        N, _, H, W = qkv.size()
...
if __name__ == "__main__":
    ...
    print("Shape of conv_out1: {}".format(conv_out1.shape))

    for name, param in augmented_conv1.named_parameters():
        print('parameter name: ', name)

    augmented_conv2 = AugmentedConv(in_channels=3, out_channels=20, kernel_size=3, dk=40, dv=4, Nh=4, relative=True, stride=1, shape=32).to(device)
